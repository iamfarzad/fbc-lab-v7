...
const normalizeWebsocketUrl = (rawValue, { fallback, enforceSecure = false, }) => {
    if (!rawValue) {
    ...
    try {
        ...
        if (!isLocalHost && url.protocol === 'wss:') {
            if (url.port && url.port !== '443') {
                console.warn(`[WEBSOCKET_CONFIG] Stripping unsupported secure port "${url.port}" from ${url.hostname}`);
                url.port = '';
            }
        }
        ...
        console.warn('[WEBSOCKET_CONFIG] Invalid WebSocket URL provided; falling back to default', error);
        return trimTrailingSlash(fallback);
    }
};
...
export const EXTERNAL_ENDPOINTS = {
    ...
    PERPLEXITY_CHAT_COMPLETIONS: process.env.NEXT_PUBLIC_PERPLEXITY_API_ENDPOINT || 'https://api.perplexity.ai/chat/completions',
};
...
const schedulingBaseUrl = trimTrailingSlash(process.env.NEXT_PUBLIC_SCHEDULING_BASE_URL || 'https://cal.com');
const schedulingEmbedBaseUrl = trimTrailingSlash(process.env.NEXT_PUBLIC_SCHEDULING_EMBED_BASE_URL || 'https://app.cal.com');
const schedulingEmbedScript = process.env.NEXT_PUBLIC_SCHEDULING_EMBED_SCRIPT || 'https://app.cal.com/embed/embed.js';
...
export const VOICE_CONFIG = {
    ...
    VISUAL_TRIGGERS: (process.env.LIVE_SERVER_VISUAL_TRIGGERS || 'screen,showing,look at,see this,dashboard,workflow')
        ...
        .filter(Boolean),
    VISUAL_INJECT_THROTTLE_MS: Math.max(0, Number.parseInt(process.env.LIVE_SERVER_VISUAL_INJECT_THROTTLE_MS || '8000', 10) || 8000),
    CONTEXT_INJECT_DEBOUNCE_MS: Math.max(0, Number.parseInt(process.env.LIVE_SERVER_CONTEXT_INJECT_DEBOUNCE_MS || '600', 10) || 600),
